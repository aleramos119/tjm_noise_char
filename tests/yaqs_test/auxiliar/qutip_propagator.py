...
class Propagator:
    ...
    def noise_model_2_qutip_noise_list(self, noise_model: NoiseModel):
    ...
    def obs_list_2_qutip_obs_list(self, obs_list: list[Observable]):
        qutip_obs_list = []

        for obs in obs_list:
            site = obs.sites  ## Only works if sites is an integer. TODO: Add support for list of sites.
            name = obs.gate.name
            if site < self.sites:
                qutip_obs_list.append(qt.tensor([qutip_operator_dict[name] if i==site else qt.qeye(2) for i in range(self.sites)]))

        return qutip_obs_list


    def init_state_2_qutip_init_state(self, init_state: MPS):

        indices = [int(np.argmax(np.isclose(a, 1+0j))) for a in init_state.tensors] # indices has size self.sites. Each element is the state of the site in the computational basis. Example: [0, 1, 0, 1] means the state is |0101>

        initial_state = qt.tensor([qt.basis(2, idx) for idx in indices])
    ...
    def hamiltonian_2_qutip_hamiltonian(self, hamiltonian: MPO):
        ...
        hamiltonian = 0
        for i in range(L-1):
            hamiltonian += -J * qt.tensor([qt.sigmaz() if n==i or n==i+1 else qt.qeye(2) for n in range(L)])
        for i in range(L):
            hamiltonian += -g * qt.tensor([qt.sigmax() if n==i else qt.qeye(2) for n in range(L)])
    ...
    def run(self, noise_model: CompactNoiseModel) -> None:
        ...
        self.qutip_obs_list = self.obs_list_2_qutip_obs_list(self.obs_list)


        self.qutip_hamiltonian = self.hamiltonian_2_qutip_hamiltonian(self.hamiltonian)
        self.qutip_noise_model = self.noise_model_2_qutip_noise_list(noise_model.expanded_noise_model)
        ...
        if self.sim_params.num_traj > 1:
            result_lindblad = qt.mcsolve(self.qutip_hamiltonian, self.qutip_initial_state, self.times, self.qutip_noise_model, self.qutip_obs_list, ntraj=self.sim_params.num_traj, options={"map": "parallel", "num_cpus": max(1, available_cpus() - 1)})

        else:
            result_lindblad = qt.mesolve(self.qutip_hamiltonian, self.qutip_initial_state, self.times, self.qutip_noise_model, self.qutip_obs_list)
