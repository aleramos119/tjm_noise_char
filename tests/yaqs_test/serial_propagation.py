...
def main_code(folder, ntraj, L, order , threshold, method, solver, req_cpus, g_rel, g_deph):
    ...
    if method == "scikit_tt":
        ...
        t, ref_traj_T, d_On_d_gk=scikit_tt_traj(sim_params)
        print("SciKit-TT method completed")

    elif method == "tjm":
        t, ref_traj_T, d_On_d_gk = tjm_traj(sim_params)

    elif method=="qutip":
        t, ref_traj_T, d_On_d_gk = qutip_traj(sim_params)


    ref_traj=ref_traj_T.T


    end_time = time.time()


    ref_traj_with_t= np.hstack((t.reshape(-1, 1), ref_traj))  # prepend




    ## Saving reference trajectory and gammas
    header =   "t  " +  "  ".join([obs+str(i)   for obs in ["x","y","z"] for i in range(L) ])

    np.savetxt(folder+"/ref_traj.txt", ref_traj_with_t, header=header, fmt='%.6f')


    return t, ref_traj, d_On_d_gk
...
if __name__=="__main__":
    ...
    # args = ["test/propagation_serial","3","3","1","1e-4","scikit_tt","exact","4","0.1","0.1"]
...
plt.plot(t, ref_traj[:, 4], label="ref_traj x0")
