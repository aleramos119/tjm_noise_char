...
def Secant_Penalized_BFGS(f, x_copy, print_to_file = False, x_history_file_name=" ", avg_len = 100, alpha=0.01, max_iterations=200, threshhold = 1e-4, max_n_convergence = 50, tolerance=1e-8, Ns=10e8, N0=10e-10):
    ...
    for i in range(max_iterations-1):
        ...
        loss, grad_new, std_loss, std_grad = f(x_new)
        ...
        if abs(loss) < tolerance:
            print(f"Loss Converged after {i} iterations.")
            break
...
for d_for in range(1,d_max+1):
    ...
    # if opt_name == "ADAM":
    adam_loss_history, adam_x_history, adam_x_avg_history, adam_update_history = ADAM_gradient_descent(loss_function, x0,  beta1 = 0.5, beta2 = 0.999, avg_len = 200, print_to_file = print_to_file, x_history_file_name=x_history_file_name, threshhold=1e-4, alpha=0.01, max_iterations=1000)
    bfgs_loss_history, bfgs_x_history, bfgs_x_avg_history, bfgs_update_history = Secant_Penalized_BFGS(loss_function, x0, print_to_file = print_to_file, x_history_file_name=" ", avg_len = 100, alpha=0.01, max_iterations=1000, threshhold = 1e-4, max_n_convergence = 50, tolerance=1e-8, Ns=10e8, N0=10e-10)
...
x_history_np = np.array(adam_x_avg_history)
...
for i in range(d):
    # plt.plot(x_history_np[:100, i],'-', color=f"C{i}", label=f"x{i+1} history")
    plt.plot(x_history_np_2[:, i],'x-', color=f"C{i}", label=f"x{i+1} history")
